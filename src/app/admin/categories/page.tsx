...
export default function AdminCategoriesPage() {
  ...
  const handleUpdate = async (id: number) => {
    ...
    try { await api.put(`/categories/${id}`, { category_name: editName }); toast.success('Kategori berhasil diperbarui'); setEditId(null); fetchCategories(); }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    setIsDeleting(true);
    try { await api.delete(`/categories/${deleteId}`); toast.success('Kategori berhasil dihapus'); fetchCategories(); }
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <EmptyState title="Belum ada kategori" message="Tambahkan kategori pertama untuk laporan." icon={<Tag className="w-8 h-8 text-slate-400" />} />
      ) : (
      ...
      <ConfirmDialog isOpen={deleteId !== null} title="Hapus Kategori" message="Apakah Anda yakin ingin menghapus kategori ini?" onConfirm={handleDelete} onCancel={() => setDeleteId(null)} isLoading={isDeleting} />
    </div>
  );
}
