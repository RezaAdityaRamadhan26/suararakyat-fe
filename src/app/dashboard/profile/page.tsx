...
import { User, Save, Lock, Calendar } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (profileData.created_at) {
        setJoinedDate(profileData.created_at);
      }
    } catch {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update auth store agar topbar juga berubah
      if (user && token) {
        setAuth({ ...user, username: updatedData.username }, token);
      }
      ...
      >
        ...
        <div className="pt-14 px-6 pb-6">
          <div className="flex items-center gap-4 text-sm text-slate-500 mb-8 pb-6 border-b border-slate-100">
            ...
            <span className="flex items-center gap-1.5">
              <span className="px-2 py-0.5 rounded-full bg-sky-50 text-sky-700 text-xs font-semibold tracking-wide border border-sky-200">
                User
              </span>
            </span>
          </div>
